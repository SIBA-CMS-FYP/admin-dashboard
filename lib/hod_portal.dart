...
class HodResponse extends StatefulWidget {
  String hodcms;
  HodResponse(this.hodcms, {Key? key}) : super(key: key);

  @override
  _HodResponseState createState() => _HodResponseState();
}

class _HodResponseState extends State<HodResponse> {
  // Future<SendWR>? _futureRequest;
  Future<TechGetReq>? studentCourseForWithdraw;
  late String hodcms = widget.hodcms;
  // var enrollId;
  @override
  initState() {
    super.initState();
  ...
  Future<void> _loadCurrentEnroll() async {
    setState(() {
      studentCourseForWithdraw = fetch_C_H(hodcms.toString());
    });
  }

  Future<void> updateisWithdraw(String cms, String Course_Code) async {
    final response = await http.post(
      Uri.parse('http://localhost:3000/getCurrent/withdrawHCResponse'),
            ...
            if (Coursesdata.hasData) {
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return cData.row[index].isHODAcept == 1
                      ? Card(
                          child: ListTile(
                              title: Text("Name " +
